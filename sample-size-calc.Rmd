...
numericInput("min_effect", 
             label = NA, value = 10, width = "100px")
```

_This means you're looking to detect a change in the conversion rate from **`r textOutput("baseline3", inline = TRUE)`** to at least **`r   textOutput("p2_value1", inline = TRUE)`**._
...
**Reference:** [Differences Between One-Tailed and Two-Tailed Test](https://stats.idre.ucla.edu/other/mult-pkg/faq/general/faq-what-are-the-differences-between-one-tailed-and-two-tailed-tests/)
...
### Not Sure What Lift Percentage to Enter?
...
### Power ($1 - \beta$): Reduce Risk of a False Negative

Set the **statistical power**. The _higher_ you set the statistical power, the _less likely_ the statistical results will return a _false negative_ (aka, a Type II error).
...
sliderInput(inputId = "power",
            ...
            min = 50, max = 100, value = 80, width = "300px", post = "%")
```

### Confidence ($1 - \alpha$): Reduce Risk of a False Positive

Set the **statistical confidence**. The higher you set the statistical confidence, the _less likely_ the statistical results will return a _false positive_ (aka a Type I error).
...
sliderInput(inputId = "confidence",
            label = div(style='width:300px;',
                        div(style='float:left;', 'MORE Likely'),
                        div(style='float:right;', 'LESS Likely')),
            min = 50, max = 100, value = 95, width = '300px', post = "%")
```
...
output$baseline2 <- renderText(paste0(input$baseline,"%"))
output$baseline3 <- renderText(paste0(input$baseline,"%"))
...
output$sample_size <- renderText({
  ...
  if(input$confidence == 100){
    results <- "100% Confidence Is Not Achievable"
  } else if(input$power == 50 & input$confidence == 50){
...
# Build the plot
output$error_matrix <- renderPlot({
  ...
  error_df <- data.frame(reality = factor(c("POSITIVE", "NEGATIVE", "POSITIVE", "NEGATIVE"),
                         ...
                         # The coin toss text. We're just going to put this in the top right
                         # quadrant. Really starting to push ggplot2 here!
                         coin_toss_text = c("", "Coin\nToss","","")
                         )
  ...
  error_gg <- ggplot(data = error_df, mapping = aes(label = label)) +
    ...
    geom_rect(aes(xmin = rect_min_good_x, xmax = rect_max_good_x,
                  ymin = rect_min_good_y, ymax = rect_max_good_y),
              fill = "#B8D8BA") +
   
    # Draw the rectangles that will show how much the risk is
    geom_rect(aes(xmin = rect_min_error, xmax = rect_max_error,
