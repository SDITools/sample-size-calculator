...
Enter values below to calculate the *minimum effect required* in an A/B test to reliably detect a difference in a desired runtime.
...
### 1. Baseline Conversion Rate and Desired Runtime
...
Enter the **desired test duration** you would need to see in order to make a decision (positive or negative):
...
output$liftmessage <- renderUI({
  if(is.na(input$baseline) | input$baseline == 0 |
    ...
    message <- HTML("<div style=\"color:red;\">Enter a baseline conversion rate and a desired runtime above.</div>")
  } else {
    message <- HTML("")
  }
...
Given the selected runtime for the test, the baseline conversion rate, and the traffic to the experience, the **minimum detectable effect** required for this runtime is:
...
# Reactive expression to calculate the min effect. We're going to use this
...
min_effect_calc <- reactive({
  ...
  if(input$power == 50 & input$confidence == 50){
    ...
    results <- tryCatch(
      ...
      calc_p2 <- calc_p2$p2/(input$baseline/100) - 1
      },
      error = function(e){
        return(-999)
      }
...
output$range_message <- renderUI({
  ...
  if(is.na(input$baseline) | input$baseline == 0 |
     is.na(input$runtime) | input$runtime == 0){
    message <- HTML("<div style=\"color:red;\">Enter a baseline conversion rate and a desired runtime above.</font>")
  } else {
    ...
    if(min_effect == -999 | min_effect == -1000){
      ...
      if(input$tail == "one.sided"){
        message = paste0("The selected runtime (", input$runtime, " days) requires a change in the ",
                         "conversion rate from <strong>", baseline, "</strong> to at least <strong>", p2_val_1,
                         "</strong> to be a detected difference.")
      } else {
        message = paste0("The selected runtime (", input$runtime, " days) requires a change in the ",
                         "conversion rate from <strong>", baseline, "</strong> to at least <strong>", p2_val_1,
                         "</strong> OR from <strong>", baseline, "</strong> to at least <strong>",
                         p2_val_2, "</strong> (in a two-tailed test) to be a detected difference.")
      }
    }
  }
...
output$mineffect_time <- renderPlot({
  ...
  days_needed <- which.min(ifelse(
    (min_effect_calc() > 0 & min_by_day$lift_pos > min_effect_calc()) |
      (min_effect_calc() < 0 & min_by_day$lift_pos < min_effect_calc()),
    NA,
    abs(min_by_day$lift_pos-min_effect_calc())))
  ...
  if(input$tail == "one.sided"){
    min_effect_df <- data.frame(yintercept = min_effect_calc(),
                                label = paste0(input$min_effect, "%"))
  } else {
    min_effect_df <- data.frame(yintercept = c(min_effect_calc(), -1*min_effect_calc()),
